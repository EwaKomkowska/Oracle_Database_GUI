...
--------------------------------------------------------
  ...
  CREATE TABLE OPLATA 
   (	IDOPLATY NUMBER(*,0) DEFAULT OPLATA_SEQ.nextval, 
	...
	/*,
	CONSTRAINT OPLATA_POMOC_FK FOREIGN KEY (POMOCDYDAKTYCZNA_IDPOMOCY, POMOCDYDAKTYCZNA_IDPRAC, POMOCDYDAKTYCZNA_IDGRUPY) 
	REFERENCES POMOCDYDAKTYCZNA (IDPOMOCY, PRZEDSZKOLANKA_IDPRAC, GRUPAPRZEDSZKOLNA_IDGRUPY)
	ON DELETE CASCADE, 

	CONSTRAINT OPLATA_ZAJECIA_FK FOREIGN KEY (ZAJECIADODATKOWE_IDZAJECIA)
	REFERENCES ZAJECIADODATKOWE (IDZAJECIA)
	ON DELETE CASCADE*/
   ) ;
--------------------------------------------------------
...
/*
	,
	CONSTRAINT POMOC_GRUPA_FK FOREIGN KEY (GRUPAPRZEDSZKOLNA_IDGRUPY)
	REFERENCES GRUPAPRZEDSZKOLNA (IDGRUPY)
	ON DELETE CASCADE,

	CONSTRAINT POMOC_OPLATA_FK FOREIGN KEY (OPLATA_IDOPLATY)
	REFERENCES OPLATA (IDOPLATY)
	ON DELETE CASCADE,

	CONSTRAINT POMOC_PRZEDSZKOLANKA_FK FOREIGN KEY (PRZEDSZKOLANKA_IDPRAC)
	REFERENCES PRZEDSZKOLANKA(IDPRAC)
	ON DELETE CASCADE*/
   ) ;
